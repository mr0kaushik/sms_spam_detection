...
class Contact {
  ...
  String getPhonesString() {
    RegExp exp = new RegExp(r"([^0-9+])");
    List<String> tPhones = [];
    String str = '';

    if (phones != null && phones.length > 0) {
      for (Item item in phones) {
        tPhones.add(item.value.replaceAll(exp, '').trim());
      }
      List<String> addresses = tPhones.toSet().toList();
  ...
  String getPrimaryAddress() {
    RegExp exp = new RegExp(r"([^0-9+])");
    if (phones != null && phones.length > 0) {
      return phones.toList()[0].value.replaceAll(exp, '').trim();
    }
    return '';
  }
